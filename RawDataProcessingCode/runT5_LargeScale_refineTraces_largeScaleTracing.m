...
for jj = 0:NFOV-1
    ...
    if exist(['Traces_LargeScale\TraceArray' FOVid '.mat'])==2
        load(['Traces_LargeScale\TraceArray' FOVid '.mat']);
        ...
        xlabel('x (um)');
        ylabel('y (um)');
        savefig(['Traces_LargeScale\TracesRefined_' FOVid '.fig']);
        save(['Traces_LargeScale\TraceArrayRefined' FOVid '.mat'],'TraceArray');
    end
end
figure(4)
bar(FinalFociCount)
...
savefig(['TracingFociCount_final_LargeScale.fig']);
