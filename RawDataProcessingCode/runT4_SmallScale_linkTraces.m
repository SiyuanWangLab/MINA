...
% this following parameter was changed from 1 to 0.5 on 191106.
NearestNeighborThreshold = 0.5; %um, distance threshold between new focus and the average position of all previous foci in the trace
...
FociCount = zeros(NumHybs,1); % initial foci count
FinalFociCount = zeros(NumHybs,1); % final count of foci in the traces
...
%                     P0 = [P0; TraceArray{j}(end,1:3)]; % build the list of end foci in all traces
        ...
        xlabel('x (um)');
        ylabel('y (um)');
        savefig(['Traces_SmallScale\Traces_' FOVid '.fig']);
...
savefig(['TracingFociCount_initial_SmallScale.fig']);
figure(4)
bar(FinalFociCount)
xlabel('Hyb number')
ylabel('Foci count after linking traces')
savefig(['TracingFociCount_middle_SmallScale.fig']);
