...
MergeClusters = {11, 13, [1 10], 7, 14, [2 3 5], [4 6], [8 9 12 15]}; 
% neutrophil and dendritic cell; EC and preB; hepatocytes; macrophage; megakaryocytes; erythroblasts; proerythroblasts; unknown.
...
ylabel('tsne2')
legend('Location','bestoutside')
saveas(gcf, 'tsne analysis with clusters (less clusters).jpg')

Matrix2_new = [];
for i = 1:max(CellTypeID_new)
    Idx = find(CellTypeID_new == i);
    Matrix2_new = [Matrix2_new; mean(Matrix(Idx,:),1)];
end
...
hold off
saveas(gcf, 'Gene expression for cell clusters (less clusters).jpg')
savefig(['Gene expression for cell clusters (less clusters).fig'])
